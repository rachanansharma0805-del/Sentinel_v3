...
// ── TRESPASS TUNING ───────────────────────────────────────────
// TRESPASS_MAX_DIST: distance in cm that triggers alarm.
// Point HC-SR04 at open doorway. Serial Monitor prints distance
// every 600ms — use that to calibrate for your exact setup.
#define TRESPASS_MIN_DIST      5     // ignore reflections < 5cm
#define TRESPASS_MAX_DIST    120     // raised: catch people just outside
#define TRESPASS_CONFIRMS      3     // IN-range readings to trigger
#define TRESPASS_MISSES_RESET  3     // OUT-of-range readings to reset
#define TRESPASS_COOLDOWN_MS   45000UL  // 45s cooldown (was 60s)
#define TRESPASS_GRACE_MS      2000UL   // 2s grace to press A first
#define TRESPASS_ESCALATE      2     // after N alarms, fire immediately
...
String studentRoll     = "";
String studentSeat     = "";
String exitMethod      = "";   // tracks exit verification method
String exitStudentName = "";   // tracks name during exit flow
...
// Trespass state variables
int           trespassConfirmCount  = 0;  // consecutive IN readings
int           trespassMissCount     = 0;  // consecutive OUT readings
unsigned long trespassFirstSeen     = 0;  // when person first entered range
int           trespassSessionCount  = 0;  // alarms fired this session (escalation)
...
void showHome() {
  ...
  trespassMissCount    = 0;
  trespassFirstSeen    = 0;
  // Note: trespassSessionCount intentionally NOT reset here
  // It tracks alarms per session for escalation logic
  stateTimeout      = 0;
  trespassConfirmCount = 0;
  buzzerStop();
  digitalWrite(LED_GREEN, LOW);
...
void mqttCallback(char* topic, byte* payload, unsigned int len) {
  ...
  if (String(topic) == "exam/face/result" && waitingFaceResult) {
    ...
    // ── SUCCESS — check if this is entry or exit ───────
    digitalWrite(LED_GREEN, HIGH);

    if (exitMethod == "rfid+face" && exitStudentName != "") {
      // This was an EXIT face scan
      String confLine = "Conf:" + String((int)conf) + " verified";
      showOLED("Exit Verified!",
               exitStudentName.substring(0, 16),
               confLine,
               "Recording exit...");
      beepSuccess();
      delay(800);
      recordExit(scannedUID, "rfid+face");
      exitMethod      = "";
      exitStudentName = "";
    } else {
      // This was an ENTRY face scan
      ...
      showOLED("ENTRY GRANTED!",
               ...
               confLine);
      beepSuccess();
      // Reset trespass cooldown — next student can approach immediately
      trespassCooldown     = 0;
      trespassConfirmCount = 0;
      trespassMissCount    = 0;
      trespassFirstSeen    = 0;
  ...
  if (String(topic) == "exam/qr/result" && waitingQRResult) {
    ...
    // FIX: Handle scanner busy — tell student to wait and retry
    if (reason == "scanner_busy_please_wait") {
      showOLED("Scanner Busy!",
               "Another scan active",
               "Please wait 20s",
               "then press 7 again");
      beepFail();
      appState = 5;  // back to fallback menu so they can retry
      return;
    }
    ...
    if (valid) {
      ...
      String errMsg =
        ...
        reason == "webcam_unavailable"       ? "Webcam not available!"  :
        "QR failed: " + reason;
...
void handleFacultyAuth() {
  ...
  if (success) {
    ...
    // New session started — reset escalation counter
    trespassSessionCount = 0;
...
void handleStudentPIN() {
  if (enteredPIN.length() < 4) {
    String header = exitMethod != "" ? "Exit PIN" : "Emergency PIN";
    showOLED(header,
             "Too short! Min 4",
             "digits needed",
             "C=confirm 3=clear");
  ...
  if (res["success"] | false) {
    ...
    if (exitMethod == "student_pin" && exitStudentName != "") {
      // Exit via PIN — record exit
      showOLED("PIN Verified!",
               name.substring(0, 16),
               "Recording exit...", "");
      beepSuccess();
      delay(500);
      recordExit(scannedUID, "student_pin");
      exitMethod      = "";
      exitStudentName = "";
    } else {
      // Entry via PIN
      String seat = res["seat_no"] | "?";
...
// ══════════════════════════════════════════════════════════════
// CORE: EXIT — matches entry method
// ══════════════════════════════════════════════════════════════

void recordExit(String uid, String method) {
  showOLED("Recording exit...", "", "", "");

  StaticJsonDocument<128> req;
  req["room_id"]     = ROOM_ID;
  req["rfid_uid"]    = uid;
  req["exit_method"] = method;
  String body;
  serializeJson(req, body);
...
void handleExitRFIDTap(String uid) {
  buzzerStop();
  scannedUID = uid;

  showOLED("Checking exit...",
           uid.substring(0, 11),
           "Please wait...", "");
  ...
  String body;
  serializeJson(req, body);
  String resp = httpPost("/student/exit-check", body);
  ...
  bool alreadyExited = doc["already_exited"] | false;
  if (alreadyExited) {
    showOLED("Already exited!",
             String(doc["student_name"] | "Student").substring(0,16),
             "Exit already logged",
             "for today");
    beepFail();
    stateTimeout = millis() + 3000;
  ...
  exitStudentName = doc["student_name"] | "Student";
  studentRoll     = doc["roll_no"]      | "?";
  exitMethod      = doc["auth_method"]  | "rfid+face";

  Serial.println("[EXIT CHECK] " + exitStudentName +
                 " needs: " + exitMethod);

  // FIX: Handle student who has no entry log today
  if (exitMethod == "no_entry_today") {
    showOLED("No Entry Found!",
             exitStudentName.substring(0, 16),
             "No entry log today",
             "Contact invigilator");
    beepFail();
  ...
  if (exitMethod == "rfid+face") {
    // Face scan required to exit
    beepSuccess();
    showOLED("Exit: Face scan",
             exitStudentName.substring(0, 16),
             "Look at CAM now",
             "B=cancel");
    delay(800);
    ...
    if (!camOnline || millis() - camLastSeen > 30000) {
      ...
      while (!camOnline && millis() - ps < 3000) {
        mqtt.loop(); delay(100);
      }
    }
  ...
  } else if (exitMethod == "student_pin") {
    // PIN required to exit
    enteredPIN = "";
    appState   = 4;
    beepPINReq();
    showOLED("Exit: Enter PIN",
             exitStudentName.substring(0, 16),
             "Enter your PIN:",
             "C=confirm 3=clear");

  } else {
    // qr_webcam or anything else — just record with RFID
    recordExit(uid, exitMethod);
  }
}

// Keep old handleExit as alias for compatibility
void handleExit(String uid) {
  handleExitRFIDTap(uid);
}

// ══════════════════════════════════════════════════════════════
// CORE: TRESPASS — fully patched loophole-free version
// ══════════════════════════════════════════════════════════════
void handleTrespass(long dist) {
  ...
  trespassMissCount    = 0;
  trespassFirstSeen    = 0;
  trespassSessionCount++;   // escalation counter

  Serial.println("[TRESPASS] CONFIRMED dist=" +
                 String(dist) + "cm alarm#" +
                 String(trespassSessionCount));

  // Immediate snapshot BEFORE alarm beeps
  // This ensures photo is taken while person is still there
  // not after 1.5 seconds of beeping
  StaticJsonDocument<128> cam;
  cam["cmd"]    = "snapshot";
  cam["reason"] = "trespass";
  cam["alarm"]  = trespassSessionCount;
  String camBody;
  serializeJson(cam, camBody);
  mqtt.publish("exam/cam/command", camBody.c_str());

  // Show OLED immediately
  showOLED("!! TRESPASS !!",
           "Alarm #" + String(trespassSessionCount),
           "Security alerted!",
           "Dist: " + String(dist) + "cm");

  // Alarm — escalates on repeat offences
  if (trespassSessionCount >= TRESPASS_ESCALATE) {
    // Louder, longer alarm for repeat trespass
    for (int i = 0; i < 15; i++) {
      digitalWrite(BUZZER_PIN, HIGH);
      digitalWrite(LED_GREEN,  HIGH);
      delay(100);
    ...
    showOLED("!! REPEAT !!", "TRESPASS ALARM",
             "Alarm #" + String(trespassSessionCount),
             "Security notified!");
  } else {
    beepTrespassAlarm();   // standard 10-beep alarm
  }
...
void loop() {
  ...
  if (appState == 0 &&
    ...
    if (dist > TRESPASS_MIN_DIST && dist < TRESPASS_MAX_DIST) {
      ...
      // ── Person is in range ──────────────────────────────
      trespassMissCount = 0;   // reset miss counter on any IN reading

      // First detection — start grace period timer
      if (trespassConfirmCount == 0 && trespassFirstSeen == 0) {
        trespassFirstSeen = millis();
        Serial.println("[TRESPASS] Person detected — grace period started");
      }

      // During grace period — legitimate student can press A
      if (millis() - trespassFirstSeen < TRESPASS_GRACE_MS) {
        Serial.println("[TRESPASS] Grace: " +
                       String(millis() - trespassFirstSeen) +
                       "ms / " + String(TRESPASS_GRACE_MS) + "ms");

      } else {
        // Grace period expired — start counting

        // Escalation: if this is a repeat trespasser this session,
        // fire immediately without needing 3 confirmations
        if (trespassSessionCount >= TRESPASS_ESCALATE) {
          Serial.println("[TRESPASS] ESCALATED — immediate fire");
          handleTrespass(dist);
          return;
        }
      ...
      if (trespassConfirmCount > 0 || trespassFirstSeen > 0) {
        trespassMissCount++;
        Serial.println("[TRESPASS] Miss " +
                       String(trespassMissCount) + "/" +
                       String(TRESPASS_MISSES_RESET));

        // Only reset after TRESPASS_MISSES_RESET consecutive
        // out-of-range readings — prevents inching loophole
        if (trespassMissCount >= TRESPASS_MISSES_RESET) {
          Serial.println("[TRESPASS] Counter fully reset");
          trespassConfirmCount = 0;
          trespassMissCount    = 0;
          trespassFirstSeen    = 0;
        }
  ...
  if (appState == 0) {
    ...
    if (key == '6') {
      ...
      showOLED("Exit Mode",
               ...
               "Tap RFID card",
               "B=cancel");
      Serial.println("[EXIT MODE] Activated");
    }
  }
  ...
  // ══════════════════════════════════════════════════════
  // STATE 4: STUDENT EMERGENCY PIN (entry) or EXIT PIN
  // ══════════════════════════════════════════════════════
  else if (appState == 4) {
    if (isSafeDigit(key)) {
      enteredPIN += key;
    ...
    if (key == '3') {
      enteredPIN = "";
      String header = exitMethod != "" ? "Exit PIN" : "Emergency PIN";
      showOLED(header, "Cleared!", "Enter PIN:", "C=confirm");
    }
    if (key == 'C') {
      // If exitMethod is set — this is an exit PIN verification
      if (exitMethod == "student_pin" && exitStudentName != "") {
        handleStudentPIN();   // verify PIN
        // If PIN accepted, record exit instead of entry
        // (handleStudentPIN publishes to /student/auth/pin
        // which logs an entry — we override with exit after)
        // For simplicity, record exit directly here after PIN check
      } else {
        handleStudentPIN();   // normal entry PIN
      }
    }
    if (key == 'B') {
      if (exitMethod != "") {
