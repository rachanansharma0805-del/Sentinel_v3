...
// ESP32-CAM (AI Thinker) Firmware v4 — OV3660 edition
// ═══════════════════════════════════════════════════════════════
//
...
//   Camera  = OV3660 on dedicated CSI bus (pins handled by esp_camera)
...
bool initCamera() {
  ...
  // Sensor tuning — OV3660 specific
  // OV3660 is physically mounted inverted on AI Thinker board
  // vflip=1 corrects the upside-down image
  // saturation reduced to avoid oversaturation in OV3660
  sensor_t* s = esp_camera_sensor_get();
  if (s->id.PID == OV3660_PID) {
    s->set_vflip(s, 1);          // OV3660 mounted inverted — MUST flip
    s->set_brightness(s, 1);     // slight boost for exam hall lighting
    s->set_saturation(s, -2);    // reduce OV3660 oversaturation
    ...
    Serial.println("[CAM] OV3660 sensor tuning applied");
  } else {
    // Fallback for OV2640 or other sensors
    ...
    s->set_hmirror(s, 0);
    s->set_vflip(s, 0);
    Serial.println("[CAM] Generic sensor tuning applied");
  }
...
void handleFaceVerify(const String& studentId) {
  ...
  delay(500);  // OV3660 needs ~500ms to settle auto-exposure
...
// ── cmd = qr_scan ─────────────────────────────────────────────
//  Tries multiple times with different preprocessing to handle
//  both printed paper QR and phone screen QR (glare, brightness)
void handleQRScan(const String& studentId) {
  ...
  // Try up to 3 captures to get a good QR read
  // Each attempt uses flash to maximise contrast
  camera_fb_t* fb = nullptr;
  for (int attempt = 0; attempt < 3; attempt++) {
    Serial.printf("[QR] Capture attempt %d/3\n", attempt + 1);

    // Use flash for QR — printed paper and phone screens
    // both benefit from controlled lighting
    fb = captureFrame(true);

    if (fb && fb->len > 0) {
      Serial.printf("[CAM] Captured %d bytes (attempt %d)\n",
                    fb->len, attempt + 1);
      break;
    }
    if (fb) esp_camera_fb_return(fb);
  ...
  // If QR not detected, try once more without flash
  // (sometimes phone screens are washed out by flash)
  bool valid = doc["valid"] | false;
  String reason = doc["reason"] | doc["error"] | "";

  if (!valid && (reason == "no_qr_detected" || reason == "no_qr_detected")) {
    Serial.println("[QR] Not detected with flash — trying without flash");
    setResolution(CAM_QR_SIZE);
    delay(200);
    fb = captureFrame(false);  // no flash this time
    if (fb) {
      resp = httpPostImage("/api/qr/scan", fb, metaStr.c_str());
      esp_camera_fb_return(fb);
...
void mqttCallback(char* topic, byte* payload, unsigned int len) {
  ...
  // ── PING from WROOM (v2) ─────────────────────────
  // WROOM sends ping before face scan to confirm CAM alive
  // CAM responds with cam_online on exam/status
  if (String(topic) == "exam/cam/ping") {
    Serial.println("[MQTT] Ping received — sending pong");
    StaticJsonDocument<128> pong;
    pong["event"]   = "cam_online";
    pong["room_id"] = ROOM_ID;
    pong["ip"]      = WiFi.localIP().toString();
    String pongStr;
    serializeJson(pong, pongStr);
    mqtt.publish("exam/status", pongStr.c_str());
    return;
  }
  ...
  if (cmd == "face_verify") {
    ...
    // QR scanning is handled by server.py laptop webcam — NOT the CAM
    // CAM ignores this command to prevent race condition where
    // CAM's fast failure arrives before server.py webcam result
    Serial.println("[CMD] qr_scan — handled by server.py webcam, CAM ignoring");
...
void reconnectMQTT() {
  ...
  if (mqtt.connect(cid.c_str())) {
    ...
    mqtt.subscribe("exam/cam/ping");        // v2: respond to WROOM pings
    Serial.println("[MQTT] Subscribed: exam/cam/command + exam/cam/ping");
...
void setup() {
  Serial.begin(115200);
  delay(500);
  Serial.println("\n[BOOT] Sentinel ESP32-CAM v4 (OV3660)");
